...
def make_data(data, bo):
    ...
    return pickle.dumps(bo)

def spectate_game(conn, game):
...
def close_connection(conn, game, name):
    global games
    try:
        del games[game]
...
def threaded_client(conn, game, spec=False):
    ...
    if not spec:
        ...
        update_data(conn, game, bo)
        
        connections -= 1
        close_connection(conn, game, name)

    else:
        spectate_game(conn, game)
